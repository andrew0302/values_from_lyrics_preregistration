---
title: 'big bad sim function data tables version'
...
- random effects of machine learning systems
- machine outputs, comprised of machine random effects and ground truth

In addition, machine observations can be treated as fixed effects
...
![appendix_table](../screenshots/appendix, Schwartz 2001.png)

Means and standard deviations retrieved from this table:
![table_2](../screenshots/table 2, Schwartz 2001.png)
I follow variable naming conventions from this tutorial: 

![name_conventions](../screenshots/table 2, De Bruine & Barr 2021.png)
...
```{r}
  ...
  # null slope correlation matrix
  rho_s = diag(1 ,11, 11)
  rho_m = diag(1 ,11, 11)
  
  # alternative slope / intercept correlation matrices
...
### Format Machine 'Observations' as Fixed Effects
...
#machine_trials <- machine_sim_function_dt(n_machines, n_trials, tau_0_m, tau_1_m, tau_2_m, rho_m, e_sd, item_ground_truth)
#machine_trials_as_fixed <- machines_as_fixed_function_dt(machine_trials, subject_trials, value_names)
```

### Treat Machines as Respondents
...
#machine_trials <- machine_sim_function_dt(n_machines, n_trials, tau_0_m, tau_1_m, tau_2_m, rho_m, e_sd, item_ground_truth)
#machine_trials_as_subjects_dt <- machines_as_subjects_function_dt(machine_trials, subject_trials)
```
...
```{r}
df_1 <- my_sim_data_dt(n_subj = 200, n_items = 50, n_machines = 4, process = "fixed", output = "both")
df_2 <- my_sim_data_dt(n_subj = 200, n_items = 50, n_machines = 4, process = "subjects")
```
