...
#response_df$item_ID <- as.factor(response_df$item_ID)


response_df %>% 
  group_by(item_ID) %>%
  dplyr::summarise(across(-c(participant_ID, row_number, writer, familiar), mean))
...
mean_dfs_1 <- lapply(response_dfs[1:2], function(x){
  ...
  dplyr::summarise(
    across(
      -c(participant_ID, row_number, writer, familiar), c(mean, sd)
      ))
})
