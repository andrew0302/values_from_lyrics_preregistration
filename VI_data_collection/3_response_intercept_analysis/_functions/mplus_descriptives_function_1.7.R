...
#fit   <- readRDS(here("VI_data_collection", "_data","intermediary_data", "mplus_participant_model_p_1.0.RDS"))
#intercept = 'item_ID'

#df <- responses_dt %>% rename(subject_ID = participant_ID)
#fit <- readRDS(here("VI_data_collection", "_data","intermediary_data", "mplus_participant_model_1.0.RDS"))

#rm(responses_dt_a, responses_dt_c, responses_dt_p, responses_dt)
...
# individual_intercepts <- get_random_intercepts_mplus(df, fit, intercept = 'item_ID')

# df    <- responses_dt_a %>% rename(subject_ID = participant_ID)
# fit   <- readRDS(here("VI_data_collection", "_data","intermediary_data", "mplus_participant_model_a_1.0.RDS"))

# random_effects_df <- assemble_ci_dfs(df, fit)
# individual_intercepts <- get_random_intercepts_mplus(df, fit, intercept = 'item_ID')

# df    <- responses_dt_c %>% rename(subject_ID = participant_ID)
# fit   <- readRDS(here("VI_data_collection", "_data","intermediary_data", "mplus_participant_model_c_1.0.RDS"))
