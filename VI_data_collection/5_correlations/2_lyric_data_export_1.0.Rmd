...
colnames(mxm_ids) <- "item_ID"
mxm_ids$V1 <- seq(0, length(mxm_ids$item_ID)-1)

# remove punctuation and EOL symbols from lyric data
export_dt$lyrics_body <- gsub(",", "", export_dt$lyrics_body)
export_dt$lyrics_body <- gsub("\"", " ", export_dt$lyrics_body)
export_dt$lyrics_body <- gsub("[\r\n]", " ", export_dt$lyrics_body)
export_dt$lyrics_body <- gsub("抰", "", export_dt$lyrics_body)
...
writeLines(export_dt$lyrics_body, path, sep = "\n")

saveRDS(mxm_ids, here("VI_data_collection", "_data", "primary_data", "mxm_ids.RDS"))
```
