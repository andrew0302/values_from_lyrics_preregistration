...
songs <- head(time_taken_table$song, 20)

# plot largest sd songs
long_diff_plot <- clicks_dt[song %in% songs] %>%
...
songs <- tail(time_taken_table$song, 20)
short_diff_plot <- clicks_dt[song %in% songs] %>%
  ggplot(., aes(x=diff, y=song, color = song)) +
  geom_boxplot(show.legend=FALSE) +
  theme_minimal() +
  xlab("Time taken (In seconds)")
...
rm(time_taken_table, songs, long_diff_plot, short_diff_plot)
```
...
model <- lmer(diff ~ (1|song) + Age, data = clicks_dt)
