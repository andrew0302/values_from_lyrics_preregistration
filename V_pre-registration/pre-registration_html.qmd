...
```{r}
#contributors <- c("Andrew M. Demetriou", "Jaehun Kim", "Sandy Manolios", "Cynthia C. S. Liem")

#roles <- c("conceptualization", "data curation", "formal analysis", "funding acquisition", "investigation", "methodology", "project administration", "resources", "software", "supervision", "validation", "visualization", "writing - original draft", "writing - review and editing")

#[CRediT](https://www.kent.ac.uk/guides/credit-contributor-roles-taxonomy#:~:text=CRediT%20(Contributor%20Roles%20Taxonomy)%20is,contribution%20to%20the%20scholarly%20output.)
```

### Overview

We aim to extend work that used natural language processing (NLP) to estimate psychological values (e.g. [@schwartz2001extending]) in social-media text [@ponizovskiy2020development]. Specifically we will explore the potential to estimate the perceived psychological values in song lyrics.
...
As in [@ponizovskiy2020development], we estimate convergent validity of the grouped NLP systems and our queztionnaire by estimating correlations with related constructs measured using Linguistic Inquiry Word Count ([LIWC](https://liwc.app); [@boyd2022development]). We will also compare the performance of word counting methods used in @ponizovskiy2020development with semantic distance methods similar to those used in e.g. @beaty2021automating.
...
Lastly, we aim to make artifacts of this project reproducible. Thus, reusable instances of the NLP systems will be available on [Replicate.com](https://replicate.com/eldrin/text-concept-similarity), de-identified data from respondents and notebooks documenting our process will be available on the [Open Science Framework](https://osf.io), including the files we used to make our [formR.org](https://formR.org) survey.
...
**Secondary Null Hypothesis**: Magnitide of these correlations show no difference.
