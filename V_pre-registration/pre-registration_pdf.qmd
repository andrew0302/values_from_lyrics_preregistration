...
It has been suggested that a rater's confidence in their annotation is a relevant indicator of reliability (although possibly orthogonal to accuracy; see @cabitza2020 ). For each set of lyrics, we will ask participants to indicate the degree to which they are confident in their ratings on a solid bar ranging from 'Extremely unconfident' to 'Extremely confident').

Our pilot study (@sec-pilot2) of 20 lyric stimuli suggests participants are 'Somewhat confident' in their responses, which provides some initial evidence of self-perceived intra-rater reliability of our procedure.
...
We aim to annotate a total of 360 lyric stimuli drawn from a pool of 2200, with approximately 25 ratings for each. This is not intended to be a fully representative sample, but rather a sufficiently large sample with which to build a ground-truthing procedure, to demonstrate its potential. Size limits were determined by estimating the smallest sample size to demonstrate the viability of our procedure, taking into account time and budgetary constraints of the research team.

Our 360 stimulus set was derived using stratified random sampling (see @sec-stratifiedsampling for details), and then a final manual screening by the research teamv(see @sec-manualscreening).
...
We followed a procedure inspired by @debruine2018determining to determine the number of ratings per stimuli. Specifically, we chose a small subset of 20 lyric samples, and had approximately 500 participants rate them all. We then randomly sub-sampled from the pool of 500 raters in increments ranging from 5 to 50 raters, and estimated Cronbach's alpha for each subsample. Our conclusions suggested conservative estimates of 25 raters per lyric stimulus. See @sec-pilot2 for details.

#### Number of Participants

We estimated how long it would take a participant to complete our lyrics questionnaire, and the time it would take to complete all questions for a single lyric stimulus on average. Data were collected during data collection for our second pilot (see @sec-pilot2).
...
library(kableExtra)
path <- here("II_rater_pilot", "intermediary_data")
#time_per_song_plot <- readRDS(here(path, "time_per_song_plot.RDS"))
#time_per_song_plot
...
Given the total of 360 lyric stimuli and the time taken per stimulus, estimated the number of participants necessary to receive approximately 25 ratings per stimuli using simulation [^2]. We thus expect to collect data for `r`n_participants\` participants. See @sec-participantsimulation for further details.
...
We will use the `Refined_dictionary.txt` file, included in the supplementary materials of @ponizovskiy2020development stored on the [Open Science Framework](https://osf.io/vy475/).

#### Pre-trained models:

We consider two commonly used pre-trained word-embedding models. `word2vec-google-news` is trained on a corpus of online news articles, including about 100 billion words. It is based on the work of @mikolov2013distributed. `GloVe-common-crawl-840B` is trained with the model suggested by @pennington2014glove using crawled large-scale web pages, including about 840 billion words.

#### Models we trained:

We will also consider word embedding models directly trained from the lyrics corpus, based on @pennington2014glove. We select models on two sets of off-line criteria:

1.  loss function on the hold-out data set and
2.  English word similarity judgment data employed from @faruqui2014community.

We select two final models that score best on each criterion to hold the generality.

#### Word-vector aggregation:

It is necessary to aggregate word vectors from each lyric into a single lyric vector to be compared to the target group of words belonging to each value. We consider two methods: 1) uniform average and 2) weighted sum. In particular, weighted sum employs the inverse-document-frequency for weighting each word vector within the lyrics @de2016representation.

### Analytic approach

Our primary hypothesis is that we will observe a correlation in ratings between participants and output from Natural Language Processing (see @sec-hypotheses and @sec-magnitudes).
...
[^3]: we refer to this as the item intercept in our notebooks

Similar to @beaty2021automating, the ratings from different NLP model / corpus setups will be linearly combined into a latent variable for each value. These are then correlated to the `stimuli intercepts`.

```{mermaid}
%%| label: fig-5
%%| fig-cap: "Latent variable representing ratings from multiple NLP model / corpus setups"
...
We further contribute three assets: firstly, we provide containerized, API-reachable interface to the models that we used to estimate semantic similarity, housed on [Replicate.com](https://replicate.com/eldrin/text-concept-similarity). Secondly, we share code notebooks written with the intention of allowing for reproducibility, replication, and extension of our work. Thirdly, we share the beginnings of a psychometric scale for assessing lyric preference intensity and expertise (@sec-lyricquestions).
...
[^5]: see table 2 in @ponizovskiy2020development
...
[^6]: see table 4 in @ponizovskiy2020development
...
1.  Set "reference distribution" for each stratum, and an empty dataset to populate:
    ...
    c)  define *B* as ta currently empty set of lyrics which we will populate
...
[^8]: see `II_manual_lyric_adjustment*.Rmd` in the `IV_survey_builder` folder
...
[^9]: See `IX_participation_estimation.ipynb` notebook in `II_rater_pilot` folder for further details
